...
class Model(nn.Module):
	...
	def calcLoss(self, adj, ancs=None, poss=None, negs=None, opt=None):
		if ancs is not None:
			uniqAncs = t.unique(ancs)
		else:
			uniqAncs =t.tensor([], dtype=t.long)
		if poss is not None:
			uniqPoss = t.unique(poss)
		else:
			uniqPoss =t.tensor([], dtype=t.long)
		...
		# loss = mainLoss + contrastDistill + regLoss + selfContrast
		# losses = {'mainLoss': mainLoss, 'contrastDistill': contrastDistill,  'regLoss': regLoss}
